...
proda_res<- imap(sampgroup_pairs,function(sampgroup_pair,contrnm){
	...
	contr=(design(proDAfitms)[g2ms,] - design(proDAfitms)[g1ms,]) - 
...
quadrantplots <- imap(sampgroup_pairs[1],function(sampgroup_pair,contrnm){
    samp1=samppair[[1]]
      samp2=samppair[[2]]
	...
	rnaseqlfc%>%head
      ...
      compdata<-compdata%>%
      	mutate(
      		sigposrna =  rnaseq_sig & (log2FoldChange>0),
      		signegrna =  rnaseq_sig & (log2FoldChange<0),
      		nosigrna = !rnaseq_sig,
      		nosigdelta = !sig,
      		delta_pos = sig & (logFC > 0),
      		delta_neg = sig & (logFC < 0),
      		set = case_when(
      ...
      compdata%>%
      	rename(rna_l2fc = log2FoldChange)%>%
      	rename(delta_l2fc = logFC)%>%
      	rename(delta_sig = sig)%>%
      	select(contrast,gene_name=gnm,rna_l2fc,rnaseq_sig,
      		ms_l2fc, delta_l2fc, delta_sig, set)%>%
      	write_tsv(tablnm)
...
alldesign%>%filter(group=='ef1')

allvoom$E%>%colnames
alldesign%>%tail
alldesign%>%group_by(group,assay)%>%tally
alldesign
...
result_names(proDAfitms)
test_diff(proDAfitms,contrast='groupMT')

test_diff(proDAfitms,contrast='`groupMT:assayMS`')
