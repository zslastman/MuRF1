...
# meoitic_gene_file <- here('ext_data/Meiotic_regulators_manual_collection_v3_ZmOrthologs_For_Dermot.xlsx')
# me_gids <- readxl::read_xlsx(meoitic_gene_file,sheet = 2)%>%set_colnames('gene_id')
# me_gids1 <- readxl::read_xlsx(meoitic_gene_file,sheet = 1)%>%set_colnames(c('gene_id2','gene_name','description','gene_id'))
# resultslistall<-map_df(resultslist,.%>%as.data.frame,.id='contrast')
# me_gids$gene_id%>%is_in(me_gids1$gene_id)
# me_gids1$gene_id%>%setdiff(me_gids$gene_id)

# rall_withm <- resultslistall%>%mutate(meiotic = gene_id%in%me_gids$gene_id)%>%filter(contrast!='Average TE')
# rall_withm%<>%mutate(contrast = str_replace(contrast,'.*change in TE - ',''))

# rall_withm%>%{qplot(data=.,y=.$log2FoldChange,x=contrast,color=meiotic,geom='blank')+facet_grid(scale='free_x',meiotic~contrast)}+geom_jitter(size=I(0.2),alpha=I(0.2))+geom_boxplot()+theme_bw()

# rall_withm%>%{qplot(data=.,x=.$log2FoldChange,color=meiotic,geom='blank')+geom_density()+facet_grid(.~contrast)}+theme_bw()

# sampleorder <- colData(dds)%>%as.data.frame%>%arrange(assay,nstage)%>%.$sample_id

# gnms <- setNames(me_gids1$gene_name,me_gids1$gene_id)
# dir.create(here('plots/geneexprplots'),showWarnings = F,rec=TRUE)

# if(!file.exists(here('data/gwidths.txt'))){
#   cfile <- fread(fcountfile)
#   lengths <- safe_hashmap(cfile$Geneid,cfile$Length)
# }
# fcountfile <- Sys.glob(here('pipeline/feature_counts/data/*/feature_counts'))%>%head(1)
	
# for(tgene in me_gids$gene_id){
#   intcounts <-   counts(dds)[tgene,] %>% enframe('sample_id','count')%>%
#     arrange(sample_id)%>%
#     arrange(match(sample_id,sampleorder))    %>%
#     separate(sample_id,c('assay','stage'))%>%
#     mutate_at(vars(assay,stage),as_factor)
  
#   genename = tgene
 

  
#   normcountdf <- assay(normcounts)[tgene,] %>% enframe('sample_id','log_reg_count')%>%
#     arrange(sample_id)%>%
#     arrange(match(sample_id,sampleorder))    %>%
#     separate(sample_id,c('assay','stage'))%>%
#     mutate_at(vars(assay,stage),as_factor)%>%
#     mutate(log_reg_count_pkbp = log_reg_count - log2(lengths[[tgene]]/1e3))
#   exprplot <- normcountdf%>%
#     qplot(data=.,x=stage,color=assay,y=log_reg_count_pkbp,geom=c('point','line'))+
#     geom_line(linetype=2,aes(x=as.numeric(stage)))+
#     ylab('Lib/Length Normalized Counts')+
#     xlab('Stage')+
#     geom_text(data=  intcounts%>%mutate(stage=as.numeric(stage))%>%
#                 mutate(stage=ifelse(assay=='RNA',stage-0.2,stage+0.2)),
#       aes(y=normcountdf$log_reg_count_pkbp%>%{min(.) - (diff(range(.))*0.2)},label=count),vjust= 1 
#     )+
#     ggtitle(gnms[tgene])+
#     theme_bw()
#   pfile <- here('plots/geneexprplots/',paste0(gnms[tgene],'_',tgene,'_exprplot.pdf'))
#   ggsave(exprplot,file=pfile,width=7,height=4)
#   normalizePath(pfile)%>%message
# }
