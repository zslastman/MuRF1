...
rule all:
  input:
    ...
    # expand('riboseqc/reports/{sample}/riboseqcreport.html', sample = ribosamples), 
    ...
    # expand('feature_counts_readrange/data/{sample}/{gcol_generegion}/{readrange}/feature_counts', sample=ribosamples, gcol_generegion='gene_id__CDS', readrange='25_35'),
    # expand('feature_counts_readrange/data/{sample}/{gcol_generegion}/{readrange}/feature_counts', sample=rnasamples, gcol_generegion='gene_id__CDS', readrange='1_1000'),
    ...
    # expand("ORFquant/{sample}/allrls.done", sample = ribosamples),
    # expand("ribotaper/{sample}/.done", sample = ribosamples)
...
rule hisat_index:
  ...
  output: directory('hisat_index/{ismasked}')
  params: 
...
rule hisat:
     ...
     shell: r"""
       ...
       MY_TMP_DIR=$(mktemp -d)
        ...
        samtools stats {output[0]} > {params.repdir}.bamstats.txt
        samtools flagstat {output[0]} > {params.repdir}.flagstat.log
        samtools idxstats {output[0]} > {params.repdir}.idxstats.log
